...
class ArticleTree extends Widget
{
    ...
    protected function renderPagetree($id, $intMargin, $protectedPage = false)
    {
        ...
        if ($objPage->numRows < 1 && $objArticle->numRows < 1)
        {
            return '';
        }
        ...
        // Check whether there are child records
        $objArticleNodes = $this->Database->prepare("SELECT * FROM tl_article WHERE pid=? ORDER BY sorting")
                ->execute($id);
        ...
        if ($objArticleNodes->numRows != 0)
        {
            while ($objArticleNodes->next())
            {
                ...
                {
                    case 'checkbox':
                        $return .= '<input type="checkbox" name="' . $this->strName . '[]" id="' . $this->strName . '_' . $objArticleNodes->id . '" class="tl_tree_checkbox" value="' . specialchars($objArticleNodes->id) . '" onfocus="Backend.getScrollOffset()"' . $this->optionChecked($objArticleNodes->id, $this->varValue) . '>';
                        break;

                    case 'radio':
                        $return .= '<input type="radio" name="' . $this->strName . '" id="' . $this->strName . '_' . $objArticleNodes->id . '" class="tl_tree_radio" value="' . specialchars($objArticleNodes->id) . '" onfocus="Backend.getScrollOffset()"' . $this->optionChecked($objArticleNodes->id, $this->varValue) . '>';
                        break;
                }
